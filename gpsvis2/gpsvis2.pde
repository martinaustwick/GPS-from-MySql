...
int toffset = int(7*3600)+int(30*60);
//display offset (h) for international time zones, etc
int doffset = 0;
...
void setup()
{
  size(500,500);
  DT = new gpsTalker();
  if (getLimits) limits = DT.getLims(x_col, y_col, dataTables[0]);
  else setLimits();
  ...
  //bgImage = loadImage("BackgroundMap New.jpg");
...
void setLimits()
{
  ...
  println("dx: " + dx + " dy: " + dy + "ratio: " + dx/dy + " 16/9: " + 16/9.0);
...
void drawType()
{
  ...
  {
    ArrayList<PVector> points = DT.getArrayList(x_col, y_col, dataTables[d], " WHERE TIME_TO_SEC("+ t_col +") <= " + t + " AND " + (t-tinc) + " < TIME_TO_SEC("+t_col+")");
